 ...
 */
	public function cleanup(Model $Model, $blockId) {
		...
		$this->TagsContent->deleteAll(array('TagsContent.tag_id' => $deleteTargetIds), false);
	}
 ...
 * @return void
 */
	protected function _cleanupLink(Model $Model, $blockId) {
		$this->loadModels([
			'Tag' => 'Tags.Tag',
			'TagsContent' => 'Tags.TagsContent'
		]);
		// TODO  つながってるタグがないリンクレコードの削除
		$deleteSql = <<<DELETESQL
DELETE tc FROM tags_contents AS tc
LEFT JOIN tags AS t ON tc.tag_id=t.id
WHERE t.id IS NULL
DELETESQL;
		$this->query($deleteSql);
		// deleteAllでやるならFindしといてdeleteAllかな



		// TODO つながってる有効なコンテンツがないリンクレコードの削除
		$this->loadModels([
			'Tag' => 'Tags.Tag',
			'TagsContent' => 'Tags.TagsContent'
		]);
